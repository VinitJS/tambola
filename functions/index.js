  ...
  activeGames[playId] = {
    interval: setInterval(async () => {
      try {
        ...
        const allTicketNumbers = new Set(Object.values(players).map(player => player.ticket).flat());
        const nextNumber = getNextNumber(
          toggle,
          Object.values(players).map(player => Array.isArray(player.ticket) ? player.ticket.filter(num => !usedNumbers.has(num)) : [] ),
...
function getNextNumber(toggle, tickets, ticketArr, restArr, usedNumbers, specialNumber) {
  if (ticketArr.every(num => num === specialNumber || usedNumbers.has(num)) && !usedNumbers.has(specialNumber)) return specialNumber;
  const filteredTickets = tickets.map(t => t.filter(n => n !== specialNumber));
  if (toggle && filteredTickets.length) return pickNumberFromTickets(filteredTickets);
  if (restArr.length) return restArr[Math.floor(Math.random() * restArr.length)];
  if (filteredTickets.length) return pickNumberFromTickets(filteredTickets);
  return null;
}