...
const Tsection = ({ game_id }) => {
    ...
    const handleJoin = async () => {
        ...
        for (let col = 0; col < 9; col++) {
            ...
            if (row_count[random_row] > 4 || (col > 1 && (ticket[random_row][col - 1] === 0 && ticket[random_row][col - 2] === 0))) {
                col--;
            } else {
        ...
        .update({
            [`players.${id}`]: {
                ...
                total_points,
                total_stars,
                ticket: ticket.flat().filter(element => Array.isArray(element)).map(([num]) => num)
            }
        }).then(() => dispatch(updateTicket({
            ticket,
            ticket_version: version
        }))).then(() => dispatch(updateClaims({
            chances_left: 6
